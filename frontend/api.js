...
const api = {
  ...
  buildImportPrompt(selectedTerms, taxonomy) {
    ...
    const jsonExample = [
      ...
      "        { \"type\": \"lab\", \"id\": \"lab\", \"title\": \"最小本地实验 / CVE 复现\", \"content\": \"## 标题\\n\\n说明段落。\\n\\n" + CB + "python\\n代码内容\\n" + CB + "\\n\\n预期输出：\" },",
    ...
    return [
      ...
      "| lab | lab | content: Markdown 格式，代码块必须用三反引号；content 为空字符串表示该术语不需要实验 | 最小实验 / CVE 复现（可选） |",
      ...
      "4. 该术语是否需要实验：如果该术语有可动手验证的漏洞机制或控制点，应设计实验；如果是纯概念/管理类术语（如安全策略、合规框架），lab 块 content 留空即可。",
      "",
      "禁止行为：",
      "- 禁止多个术语使用几乎相同的定义句式。",
      ...
      "",
      "每个术语都必须体现自己的独立机制、独立风险和独立防御控制点。实验按需提供。",
      "",
      "## 批量生成质量约束",
      "",
      batchQualityNote,
      "",
      "## lab 块（可选）Markdown 格式",
      "",
      CB + "markdown",
      labExample,
      CB,
      ...
      "",
      "3. lab 块（可选，按需生成）：",
      "   - 先判断该术语是否需要实验：如果有可动手验证的漏洞机制（如注入、溢出、绕过），则生成实验；如果是纯概念/管理类术语（如安全策略、合规框架、威胁建模方法论），lab 块 content 留空。",
      "   - 实验形式灵活：",
      "     - **0→1 最小本地实验**：用最短代码演示漏洞触发点和修复控制点，必须同时包含错误写法与修复写法。",
      "     - **CVE 复现**：针对该术语对应的某个真实 CVE，提供简化版本地复现步骤和代码。",
      "   - 代码要短，但必须体现真实控制点。",
      "",
      ...
      "- 每个术语是否都有 definition、diagram、cases、confusions、related_terms、references（lab 可选）。",
      "- 多个术语之间是否避免了定义、实验同质化。",
      "- 是否没有编造 URL。",
      "",
